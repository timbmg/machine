...
echo "\n\nTest training with hard attention, and rolled RNN with EOS in data, but EOS ignored"
python train_model.py --train $LOOKUP_HARD_ATTN_WITH_EOS --dev $LOOKUP_HARD_ATTN_WITH_EOS --output_dir $EXPT_DIR --print_every 50 --embedding_size $EMB_SIZE --hidden_size $H_SIZE --rnn_cell $CELL2 --attention 'post-rnn' --teacher_forcing_ratio 1 --epoch $EPOCH --save_every $CP_EVERY --attention_method 'hard'  --ignore_output_eos
ERR=$((ERR+$?)); EX=$((EX+1))
