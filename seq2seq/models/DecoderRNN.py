...
class DecoderRNN(BaseRNN):
    ...
    def forward_step(self, input_var, hidden, encoder_outputs, function, step=-1):
        """
        ...
        if self.use_attention == 'pre-rnn':
            ...
            if isinstance(hidden, tuple): # for GRU hidden=h, for LSTM (h, c)
                h, c = hidden
            # Apply the attention method to get the attention vector and weighted context vector. Provide decoder step for hard attentive guidance
